...
usertrap(void)
{
  ...
  // TODO: page fault handling
    ...
    if (v == 0) {
        printf("usertrap: failed to find VMA for va=0x%p pid=%d\n", va, p->pid);
        goto err;
    }
    ...
    void *pa = kalloc();
    if (pa == 0) {
        panic("usertrap: kalloc failed during lazy allocation");
    }
    memset(pa, 0, PGSIZE);
    ...
    if (mappages(p->pagetable, va, PGSIZE, (uint64)pa, perm) < 0) {
        panic("usertrap: mappages failed during lazy allocation");
    }
} else {
err:
