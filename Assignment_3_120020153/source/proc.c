...
freeproc(struct proc *p)
{
  ...
  // 解除 VMA 的页表映射并释放 VMA 结构
  for(int i = 0; i < VMASIZE; i++) {
      if(p->vma[i].valid) {
          uvmunmap(p->pagetable, p->vma[i].vastart, p->vma[i].sz / PGSIZE, 1);
          p->vma[i].valid = 0;
          p->vma[i].f = 0;  // 已经在 exit() 中关闭
      }
  }
    ...
    *(np->trapframe) = *(p->trapframe);
    ...
    for(i = 0; i < VMASIZE; i++) {
        if(p->vma[i].valid) {
            ...
            np->vma[i].vastart = p->vma[i].vastart;
